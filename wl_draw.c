...
/* Know from the id if the actor is dead */
int ActorIdDead(int id)
{
        ...
        {
                if (obj->id == id) {
                        return ActorDead(obj);
                }
        }

        /* Non-existant object id: we assume dead */
        return 1;
}
...
static void DrawScaleds()
{
	...
	char *source;
	char *target;
	struct conntrack_t *list;/* 	list = ct_list_get(16); */
