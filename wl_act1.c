...
void OpenDoor(int door, int ctx_track_refresh)
{
	if (doorobjlist[door].action == dr_open)
		doorobjlist[door].ticcount = 0;			// reset open time
	else {
                if ( ctx_track_refresh == DOOR_REFRESH ) {
                        //printf("Open the Door, we must now refresh the connections trackings\n");
                        //ct_list_random();
                        ct_list_create();
                }
