...
int LoadTheGame(char *fn, int dx, int dy)
{
	...
	while (1) {
		...
		switch (new->obclass)
		  {
		  case guardobj:
		    printf("You loaded a guard from a file!\n");
		    break;

		  case officerobj:
		    printf("You loaded an officer from a file!\n");
		    break;

		  case mutantobj:
		    printf("You loaded a mutant from a file!\n");
		    break;

		  case ssobj:
		    printf("You loaded a ss from a file!\n");
		    break;

		  default:
		    printf("Unhandled object!\n");
		  }
...
int WolfMain(int argc, char *argv[])
{
	...
	if (getuid() != 0) {
		Quit("you have to run Wolfotrack as root!");
	}
