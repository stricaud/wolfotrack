...
static short int sndbuf_tmp[512];
//static short int musbuf_tmp[256];
static int sndbuf_tmp_len = 0;

void SoundThread(void *data, Uint8* buf, int len)
//static void *SoundThread(void *data)
{
	...
	if(sndbuf_tmp_len)
	{
		if(len < sndbuf_tmp_len)
			printf("beurk..\n");
		memcpy(buf, sndbuf_tmp, sndbuf_tmp_len);
		len -= sndbuf_tmp_len;
		buf += sndbuf_tmp_len;
		sndbuf_tmp_len = 0;
	}
...
#define alWave		0xE0
...
#ifdef MUSIC_WORKS // it doesn't works
		if (NewMusic != -1) {
			NewMusic = -1;
		...
		else
		{
			...
			// SDL doesnt expect new datas anymore, so we save the current sample
			sndbuf_tmp_len = sizeof(sndbuf) - len;
			memcpy(sndbuf_tmp, sndbuf, sndbuf_tmp_len);
		}
	}
}
...
void SD_Startup()
{
	...
	if(Mix_OpenAudio(	44100,			// Hz
				MIX_DEFAULT_FORMAT,	// 16 bits
				MIX_DEFAULT_CHANNELS,	// Stereo
				sizeof(sndbuf)		// Chunks size
			   	) < 0 )
	{
		fprintf(stderr, "Unable to initialize sound\n");
		fprintf(stderr, "SDL: %s\n", SDL_GetError());
	...
	Mix_HookMusic(SoundThread, NULL);
